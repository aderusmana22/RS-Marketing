                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select" name="item_master_id" required onChange="changeItemDetailCode(this)">
                                ...
                                @foreach($itemMasters as $master)
                                    <option value="{{ $master->id }}" data-item-detail-code="{{ $master->parent_item_name }}">
                                       {{ $master->parent_item_code }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
...
@push('scripts')
    ...
    <script>
        ...
        function openEditModal(id) {
            ...
            var modalDiv = document.getElementById('editdetailModalDiv');
            var newEditModal = '';
            var detail = details[id];
                var updateUrl = "{{ route('item-detail.update', ':id') }}".replace(':id', detail.id);
                newEditModal += `
                    <div class="modal fade" id="edititemdetailModal${id}" tabindex="-1" role="dialog"
